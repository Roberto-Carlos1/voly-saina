...
public class CommandeClientService {
     ...
     */
    @Transactional
    public void mouvementStock(Commande commande, List<LigneCommande> lignesCommande) {
        ...
        for (LigneCommande lc : lignesCommande) {
            ...
            Produit p = produitService.findById(lc.getProduit().getIdProduit())
                    .orElseThrow(
                            () -> new IllegalStateException("Produit introuvable: " + lc.getProduit().getIdProduit()));
            ...
            if (p.getStock().compareTo(quantite) < 0) {
                throw new IllegalStateException("Stock insuffisant pour " + p.getNom() + " (stock=" + p.getStock()
                        + ", demandé=" + quantite + ")");
            }
        }

        // 2) Décrémenter stock + tracer MouvementStock
        TypeMouvementStock typeSortie = typeMouvementStockRepositoryAdapter
                .findByCodeOrThrow(TYPE_MOUVEMENT_STOCK_SORTIE);
        for (LigneCommande lc : lignesCommande) {
            Produit p = produitService.findById(lc.getProduit().getIdProduit())
                    .orElseThrow(
                            () -> new IllegalStateException("Produit introuvable: " + lc.getProduit().getIdProduit()));
            ...
            if (nouveauStock.compareTo(BigDecimal.ZERO) < 0) {
                throw new IllegalStateException(
                        "Stock insuffisant au moment du décrément pour " + p.getNom() + " (stock=" + p.getStock()
                                + ", décrément=" + quantite + ")");
            }
    ...
    /**
     * Petit adaptateur pour éviter d'avoir à lire TypeMouvementStockRepository à
